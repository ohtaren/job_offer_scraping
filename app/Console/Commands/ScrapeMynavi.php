...
class ScrapeMynavi extends Command
{
    ...
    private function saveUrls()
    {
        foreach (range(1, $this::PAGE_NUM) as $num) {
            ...
            $urls = $crawler->filter('.cassetteRecruit_copy > a')->each(function ($node) {
                $href = $node->attr('href');
                return [
            ...
            DB::table('mynabi_urls')->insert($urls);
            sleep(30);
        }
    }
    ...
    {
        $features = $crawler->filter('.cassetteRecruit_attribute.cassetteRecruit_attribute-jobinfo .cassetteRecruit_attributeLabel span')
            ->each(function ($node) {
                return $node->text();
            });
